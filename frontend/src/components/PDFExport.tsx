...
export default function PDFExport() {
    ...
    const generatePDF = useCallback(() => {
        ...
        // ── Research Sources ────────────────────────────────────
        if (result.researchSources) {
            addSection('📚 Additional Research & Primary Sources');
            doc.setFontSize(8);
            doc.setFont('helvetica', 'normal');
            doc.setTextColor(100, 100, 100);
            doc.text('Every number in this report can be verified. Links to primary data sources:', margin, y);
            y += 5;

            const categoryLabels: Record<string, string> = {
                financialData: '📈 Financial Data',
                technicalAnalysis: '📊 Technical Analysis',
                secFilings: '📋 SEC Filings & Regulatory',
                financialStatements: '💰 Financial Statements',
                companyResearch: '🏢 Company Research',
                newsSentiment: '📰 News & Sentiment',
                analystData: '🎯 Analyst & Institutional',
                valuationReferences: '🧮 Valuation References',
                earningsDividends: '🔬 Earnings & Dividends',
                riskCompliance: '🛡️ Risk & Compliance',
            };

            for (const [category, links] of Object.entries(result.researchSources)) {
                if (!Array.isArray(links) || links.length === 0) continue;
                checkPageBreak(10 + links.length * 5);

                doc.setFont('helvetica', 'bold');
                doc.setFontSize(9);
                doc.setTextColor(14, 165, 233);
                doc.text(categoryLabels[category] || category, margin, y);
                y += 5;

                doc.setFont('helvetica', 'normal');
                doc.setFontSize(7);
                doc.setTextColor(60, 60, 60);
                for (const link of links as { label: string; url: string }[]) {
                    checkPageBreak(5);
                    doc.text(`→ ${link.label}`, margin + 3, y);
                    doc.setTextColor(14, 165, 233);
                    const urlText = link.url.length > 80 ? link.url.substring(0, 77) + '...' : link.url;
                    doc.textWithLink(urlText, margin + 5, y + 3, { url: link.url });
                    doc.setTextColor(60, 60, 60);
                    y += 7;
                }
                y += 2;
            ...
            doc.setFontSize(7);
            doc.setTextColor(150, 150, 150);
            doc.text('Sources auto-generated by Nipun AI — verify all data independently.', margin, y);
            y += 6;
        }
